...
class SetUpDjangoDir:
    ...
    def _setup_django_urls_file(self):
        django_urls_file = os.path.join(self.django_project_folder, names.DJANGO_URLS_FILE)
        with open(django_urls_file, "w") as urls_file:
            urls_file.write(content.DJANGO_URLS_CONTENT)

    def _setup_django_settings_file(self):
        dj_settings_file = os.path.join(self.django_project_folder, names.DJANGO_SETTINGS_FILE)
        with open(dj_settings_file, "w") as settings_file:
            settings_file.write(content.write_django_settings_content(self.django_project_folder))
    ...
    def _setup_django_asgi_file(self):
        ...
        with open(dj_asgi_file, "w") as settings_file:
            settings_file.write(content.write_django_asgi_content(self.django_project_folder))
    
    def _setup_django_wsgi_file(self):
        dj_asgi_file = os.path.join(self.django_project_folder, names.DJANGO_WSGI_FILE)
        with open(dj_asgi_file, "w") as settings_file:
            settings_file.write(content.write_django_wsgi_content(self.django_project_folder))
